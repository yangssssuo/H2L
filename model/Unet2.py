...
class Attention(nn.Module):
    def __init__(self,n_embd,n_head) -> None:
        ...
        self.attention = nn.MultiheadAttention(embed_dim=n_embd,num_heads=n_head,batch_first=True)
...
if __name__ == "__main__":
    ...
    # aaa = nn.Conv1d(in_channels=1, out_channels=16, kernel_size=3)
    ...
    # ls_attn = attn_w.squeeze().detach().numpy().tolist()
    # sns.heatmap(ls_attn,cbar_kws={'orientation': 'horizontal',"shrink": 0.3},xticklabels=False, yticklabels=False)

    attn_sum = torch.sum(attn_w.squeeze(),dim=-1)
