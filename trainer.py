...
class Trainer:
    def __init__(self,model,train_set,valid_set,test_set,batchsize=64,num_workers=16,Lr=0.01,a=1,b=100) -> None:
    ...
    def train_iterations(self,mode='train'):
        ...
        self.model.train()
        losses = []
        attnn = torch.zeros(124,124).cuda()
        len_loader = len(self.train_loader) - 1 
        for i,data in enumerate(self.train_loader):
            # data = Variable(data)
            tgt = torch.ones(1).cuda()
            self.optimizer.zero_grad()
            # in_data = data[0].to(torch.float32).cuda()
            in_data = Variable(data[0].to(torch.float32).cuda())
            ...
            ori_data = Variable(data[1].to(torch.float32).cuda())
    ...
    def draw_hot(self,data,epoch):
        ...
        sns.heatmap(data,xticklabels=False, yticklabels=False,square=True,cmap='coolwarm')
        plt.savefig(f'figs/heat{epoch}.png')
        torch.save(data,f'attn_w/attn_{epoch}.pt')
        plt.close()
    ...
    def draw_spec(self):
        ...
        with torch.no_grad():
            ...
            for i,data in enumerate(loader):
                ...
                torch.save(attn_w,f'val_attn/{i}.pt')
